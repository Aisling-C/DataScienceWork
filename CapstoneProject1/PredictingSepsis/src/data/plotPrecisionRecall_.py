def Plot_PR(Model, X_train, y_train, X_test, y_test, title, auc, precision_recall_curve, np, plt, figsz):
    ...
    ix = np.argmax(fscore)
    plt.plot(recall, precision, color='red', label='Train')
    plt.scatter(recall[ix], precision[ix], marker='o', color='darkred', label='Best Train F1', s=100)
    ...
    fscore = np.nan_to_num(fscore)
    ix = np.argmax(fscore)
    plt.plot(recall, precision, color='blue', label='Test')
    plt.scatter(recall[ix], precision[ix], marker='^', color='darkblue', label='Best Test F1', s=100)
