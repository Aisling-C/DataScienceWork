...
SELECT MAX(MONTH(starttime), MIN(MONTH(starttime)
FROM Bookings
...
SELECT name, SUM(revenue) AS totalrevenue
        ...
        FROM (SELECT *
            FROM Bookings as b
            JOIN Facilities as f
                USING (facid)) AS subquery) AS subquery_2
        GROUP BY name
...
SELECT m1.surname, m1.firstname, m2.surname AS 'Recommender Surname', m2.firstname AS 'Recommended Firstname'
FROM Members as m1
JOIN Members as m2
...
FROM Facilities 
JOIN (SELECT facid FROM Bookings WHERE memid>0) AS temp
 USING (facid)
GROUP BY name
ORDER BY usage DESC;
...
SELECT name, month, COUNT(*) as 'usage'
FROM Facilities 
JOIN (SELECT facid, strftime('%m', starttime) as month FROM Bookings WHERE memid>0) AS temp
 USING (facid)
GROUP BY name, month
ORDER BY name, month;
